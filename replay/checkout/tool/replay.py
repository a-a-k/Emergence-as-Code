...
try:
    ...
    raise SystemExit(
        ...
        "Install the dependency from replay/checkout/requirements.txt."
    ) from exc


REPLAY_ROOT = Path(__file__).resolve().parents[1]
REPO_ROOT = Path(__file__).resolve().parents[3]
CHECKOUT_ROOT = REPO_ROOT / "examples" / "checkout"
FIXTURES = REPLAY_ROOT / "fixtures"
GENERATED = REPLAY_ROOT / "generated"
...
def rel(path: Path) -> str:
    path = path.resolve()
    return Path(os.path.relpath(path, REPLAY_ROOT)).as_posix()
...
def write_summary_md(summary_path: Path, rows: list[dict]) -> None:
    ...
    lines.extend(
        [
            ...
            "",
            "These files are generated by `python replay/checkout/tool/replay.py`.",
        ]
    )
